...
describe("validateTx", () => {
  test("it should validate matching transactions", () => {
    const meshTx = new MeshTx(meshAddress, amount, txHash, 1, "TxHandled");
  ...
  test("it should detect differing tx_hashes", () => {
    const meshTx = new MeshTx(meshAddress, amount, "0x999", 1, "TxHandled");
    const errors = validator.validateTx(meshTx, ethTx);
  ...
  test("it should detect differing mesh_addresses", () => {
    const meshTx = new MeshTx("5EINVALID", amount, txHash, 1, "TxHandled");
    const errors = validator.validateTx(meshTx, ethTx);
...
const ethTx = new EthTx(1, "0x123", meshAddress, amount, txHash, "0x123", "12");
